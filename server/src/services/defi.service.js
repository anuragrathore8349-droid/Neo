...
class DefiService {
  ...
  async getProtocols () {
    ...
    const protocols = [aave, uniswap, curve, lido]
      .filter(r => r.status === 'fulfilled' && r.value)
      .map(r => r.value);

    const result = protocols.length ? protocols : this.getBasicProtocols();
    this.setCache('protocols', result);
    return result;
  }
  ...
  async getPositions (userId) {
    return DefiPosition.find({ userId });
  }
  ...
  async getStakingPositions (userId) {
    try {
      const positions = await DefiPosition.find({ userId, type: 'staking' });
  ...
  // ─── LIQUIDITY POOLS ─────────────────────────────────────────────────────
  async getLiquidityPools (protocol, token, userId) {
    ...
    return pools.map(pool => ({ ...pool, myLiquidity: userPositions[pool.id] || '$0' }));
  }
