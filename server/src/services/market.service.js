...
const cryptoSymbols = symbols.filter(symbol => {
  ...
  async getTrendingAssets(options = {}) {
    try {
      ...
      if (redisClient && typeof redisClient.get === 'function') {
        ...
        if (cachedTrending) {
          const allTrending = JSON.parse(cachedTrending);
          const items = allTrending.slice(skip, skip + limit);
          return { items, total: allTrending.length };
        }
      }

      const allTrending = await this.fetchTrendingAssets();

      if (redisClient && typeof redisClient.setEx === 'function') {
        await redisClient.setEx(
          cacheKey,
          this.CACHE_DURATION.TRENDING,
          JSON.stringify(allTrending)
        );
      }
  ...
  async searchAssets(query, type, options = {}) {
    try {
      const { limit = 50, skip = 0 } = options;
      let results = [];
  ...
  async fetchForexPrices(symbols) {
    // Delegate to the working exchangerate.host implementation
    return this.getForexPrices(symbols).then(results => {
      const flat = {};
      for (const [sym, data] of Object.entries(results)) {
        flat[sym] = data?.price ?? null;
      }
      return flat;
    });
  }
  ...
  async getWatchlist(userId, options = {}) {
    try {
      const { limit = 50, skip = 0 } = options;
      const watchlist = await Watchlist.findOne({ userId });
      const symbols = watchlist?.symbols || [];
      const total = symbols.length;
      const items = symbols.slice(skip, skip + limit);
      return { items, total };
    } catch (error) {
  ...
  async getPriceAlerts(userId, options = {}) {
    try {
      const { limit = 50, skip = 0 } = options;
      const query = { userId, active: true, triggered: false };
      const total = await MarketAlert.countDocuments(query);
      const items = await MarketAlert.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit);
      return { items, total };
