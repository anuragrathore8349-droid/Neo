...
async rebalancePortfolio(req, res, next) {
  try {
    ...
    res.json({
      ...
      data: result,
      message: dryRun 
        ? 'Rebalance preview generated. Review recommendations above.'
    ...
    if (error.message.includes('at least 2 assets')) {
  ...
  async importFromCSV(req, res, next) {
    try {
      if (!req.file) {
        return res.status(400).json({ status: 'error', message: 'No CSV file provided' });
      }

      const result = await portfolioService.importAssetsFromCSV(
        req.user.userId,
        req.file.buffer
      );
      ...
      res.json({
        status: 'success',
        data: result,
        message: `Successfully imported ${result.importedCount} assets`
      });
    } catch (error) {
      next(error);
    }
