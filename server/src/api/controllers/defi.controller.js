...
class DefiController {
  ...
  async getProtocols(req, res, next) {
    try {
      ...
      const result = await defiService.getProtocols({ limit, skip });
      res.json({
        status: 'success',
        data: result.items,
  ...
  async getPositions(req, res, next) {
    try {
      const { limit = 50, skip = 0 } = req.validatedData?.query || {};
      const result = await defiService.getPositions(req.user.userId, { limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
  ...
  async getStakingPositions(req, res, next) {
    try {
      const { limit = 50, skip = 0 } = req.validatedData?.query || {};
      const result = await defiService.getStakingPositions(req.user.userId, { limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
  ...
  async getLiquidityPools(req, res, next) {
    try {
      const { protocol, token, limit = 50, skip = 0 } = req.validatedData.query || {};
      const result = await defiService.getLiquidityPools(protocol, token, req.user?.userId, { limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
  ...
  async getYieldFarms(req, res, next) {
    try {
      const { limit = 50, skip = 0 } = req.validatedData?.query || {};
      const result = await defiService.getYieldFarms(req.user?.userId, { limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
