...
class AddressBookController {
  async getAddressBook(req, res, next) {
    try {
      const { limit, skip } = req.validatedData?.query || { limit: 50, skip: 0 };
      const entries = await addressBookService.getAddressBook(req.user.userId, { limit, skip });
      res.json({
        status: 'success',
        data: entries.items,
        pagination: { limit, skip, total: entries.total }
      });
    } catch (error) {
      next(error);
    }
