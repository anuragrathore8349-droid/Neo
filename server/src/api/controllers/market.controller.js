...
class MarketController {
  ...
  async getTrendingAssets(req, res, next) {
    try {
      ...
      const result = await marketService.getTrendingAssets({ limit, skip });
      res.json({
        status: 'success',
        data: result.items,
  ...
  async searchAssets(req, res, next) {
    try {
      const { query, limit = 50, skip = 0 } = req.validatedData.query || {};
      const result = await marketService.searchAssets(query, { limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
  ...
  async getAvailableAssets(req, res, next) {
    try {
      const { limit = 50, skip = 0 } = req.validatedData?.query || {};
      const result = await marketService.getAvailableAssets({ limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
  ...
  async getWatchlist(req, res, next) {
    try {
      const { limit = 50, skip = 0 } = req.validatedData?.query || {};
      const result = await marketService.getWatchlist(req.user.userId, { limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
  ...
  async getPriceAlerts(req, res, next) {
    try {
      const { limit = 50, skip = 0 } = req.validatedData?.query || {};
      const result = await marketService.getPriceAlerts(req.user.userId, { limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
