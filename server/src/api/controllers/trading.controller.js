...
class TradingController {
  ...
  async getPaperTradeHistory(req, res, next) {
    try {
      const { symbol, limit = 50, skip = 0 } = req.validatedData.query || {};
      const result = await paperTradingService.getPaperTradeHistory(
        req.user.userId,
        symbol,
        limit,
        skip
  ...
  async getOpenOrders(req, res, next) {
    try {
      const { symbol, limit = 50, skip = 0 } = req.validatedData.query || {};
      const result = await tradingService.getOpenOrders(req.user.userId, symbol, { limit, skip });
      res.json({
        status: 'success',
        data: result.items,
        pagination: { limit, skip, total: result.total }
  ...
  async getOrderHistory(req, res, next) {
    try {
      const { symbol, from, to, limit = 50, skip = 0 } = req.validatedData.query || {};
