...
const AnalyticsDashboard: React.FC<Props> = ({ data: propData, timeframe: propTimeframe }) => {
  ...
  const [correlationData, setCorrelationData] = useState<{ symbols: string[]; matrix: number[][] } | null>(null);

  const fetchAnalytics = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const [perfRes, riskRes, assetsRes, corrRes] = await Promise.allSettled([
        ...
        apiFetch<any>('/api/analytics/correlation'),
      ]);
      ...
      if (corrRes.status === 'fulfilled') {
        setCorrelationData((corrRes as any).value?.data || null);
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    // If parent already fetched data, use it
    if (propData) {
      try {
        // Map propData.performance → perfData
        const perf = propData.performance;
        if (perf?.metrics?.returns && Array.isArray(perf.metrics.returns)) {
        ...
        // Map propData.risk → riskData
        const risk = propData.risk;
        if (risk?.assets && Array.isArray(risk.assets)) {
          setRiskData(risk.assets.slice(0, 6));
        }

        // Map propData.allocation → pieData
        const allocation = propData.allocation;
        if (allocation?.assets && Array.isArray(allocation.assets)) {
        ...
        console.error('Failed to map prop data:', err);
      }
    }

    // Otherwise fetch internally (standalone mode)
    fetchAnalytics();
  }, [propData, fetchAnalytics]);

  // Sync local timeframe with prop timeframe changes
  useEffect(() => {
    if (propTimeframe) {
            ...
            onClick={async () => {
              ...
              try {
                const rows = [
                  ...
                  ...perfData.map(p => [p.month, p.portfolio, p.portfolioReturn || 0])
                ];
