...
export interface PortfolioSummary {
  portfolioId?: string;  // Added for WebSocket subscription
  totalValue: number;
  dailyChange: number;
...
/**
 * Get rebalance recommendations without applying
 */
export async function getRebalancePreview(portfolioId: string, objective: string = 'sharpe'): Promise<{ status: string; data?: RebalanceResult; message?: string }> {
  try {
    ...
    return {
      status: 'error',
      message: error?.message || 'Failed to get rebalance preview'
    };
  }
}
...
export async function applyRebalance(portfolioId: string, objective: string = 'sharpe'): Promise<{ status: string; data?: RebalanceResult; message?: string }> {
  try {
    ...
    return {
      status: 'error',
      message: error?.message || 'Failed to apply rebalance'
    };
  }
}

/**
 * Import portfolio assets from a CSV file
 * 
 * CSV Format (headers required):
 * symbol, amount, purchase price, purchase date, [optional: name, type, notes]
 * 
 * Example:
 * symbol,amount,purchase price,purchase date,type
 * BTC,0.5,45000,2024-01-15,crypto
 * AAPL,10,150,2024-01-10,stock
 */
...
export async function importAssetsFromCSV(csvFile: File): Promise<{ status: string; data?: CSVImportResult; message?: string }> {
  try {
    const formData = new FormData();
    formData.append('file', csvFile);

    const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/portfolio/import`, {
      method: 'POST',
      headers: {
